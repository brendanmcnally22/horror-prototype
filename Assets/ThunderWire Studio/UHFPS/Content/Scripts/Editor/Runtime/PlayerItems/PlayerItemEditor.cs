    ...
    public class PlayerItemEditor<T> : Editor where T : PlayerItemBehaviour
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (EditorDrawing.BeginFoldoutBorderLayout(foldoutProperty, headerContent))
            {
                ...
                if (EditorDrawing.BeginFoldoutToggleBorderLayout(new GUIContent("Hint Controls"), Properties["EnableHintControls"]))
                {
                    using (new EditorGUI.DisabledGroupScope(!Properties.BoolValue("EnableHintControls")))
                    {
                        Properties.Draw("HintControls", 1);
                    }
                    EditorDrawing.EndBorderHeaderLayout();
                }
