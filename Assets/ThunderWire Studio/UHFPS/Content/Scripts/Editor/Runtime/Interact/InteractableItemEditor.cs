            ...
            serializedObject.Update();
            {
                ...
                using (new EditorDrawing.BorderBoxScope())
                {
                    ...
                    if (interactableTypeEnum == InteractableItem.InteractableTypeEnum.InventoryItem)
                    {
                        ...
                        {
                            SerializedProperty dataType = Properties.GetRelative("ItemCustomData.Type");
                            SerializedProperty itemAttributes = Properties.GetRelative("ItemCustomData.ItemAttributes");
                            SerializedProperty jsonData = Properties.GetRelative("ItemCustomData.JsonData");

                            EditorGUILayout.PropertyField(dataType);
                            ItemCustomData.DataType dataTypeEnum = (ItemCustomData.DataType)dataType.enumValueIndex;

                            switch(dataTypeEnum)
                            {
                                case ItemCustomData.DataType.Json:
                                    EditorGUILayout.PropertyField(jsonData);
                                    break;
                                case ItemCustomData.DataType.Attributes:
                                    EditorGUILayout.PropertyField(itemAttributes);
                                    break;
                            }
