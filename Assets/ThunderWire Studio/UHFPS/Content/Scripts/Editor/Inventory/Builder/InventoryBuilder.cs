...
namespace UHFPS.Editors
{
    public class InventoryBuilder : EditorWindow
    {
        ...
        [SerializeField]
        private TreeViewState<int> itemsViewState;
        ...
        public void Show(InventoryDatabase inventory)
        {
            ...
            if (Inventory.HasReference)
                itemSlotSize = Inventory.Instance.settings.cellSize;
        }

        private void InitializeTreeView()
        {
            builderData = new BuilderData(inventory);
            itemsViewState = new TreeViewState<int>();
            ...
            else if (selection.Selection is BuilderItem item)
            {
                ...
                builderData.SerializedObject.Update();
                {
                    ...
                    using (new EditorDrawing.IconSizeScope(14))
                    {
                        ...
                        GUIContent itemViewTitle = EditorGUIUtility.TrTextContentWithIcon("Item Preview", "GridLayoutGroup Icon");
                        using (new EditorDrawing.BorderBoxScope(itemViewTitle, roundedBox: true))
                        {
                            const float kPropertiesCount = 4;
                            const float kPropertySpaces = kPropertiesCount - 1;
                            const float kFieldLabelWidth = 100f;

                            float hLine = EditorGUIUtility.singleLineHeight;
                            float hSpace = EditorGUIUtility.standardVerticalSpacing;
                            float propSpace = hLine + hSpace;

                            float previewSize = hLine * kPropertiesCount + hSpace * kPropertySpaces;
                            Rect itemPreviewRect = EditorGUILayout.GetControlRect(false, previewSize);

                            Rect propertyRect = itemPreviewRect;
                            propertyRect.height = hLine;
                            propertyRect.xMin = 15f + previewSize + hSpace;

                            // ITEM PREVIEW
                            int slotSize = (int)itemSlotSize;
                            itemPreviewRect.width = previewSize;
                            itemPreviewRect.height = previewSize;
                            InventoryItemPreview.ShowItemSlotPreview(properties, itemPreviewRect, slotSize);

                            // WIDTH
                            Rect widthRect = propertyRect;
                            EditorGUI.LabelField(widthRect, "Width");
                            widthRect.xMin += kFieldLabelWidth;
                            var width = properties["Width"];
                            width.intValue = (ushort)EditorGUI.Slider(widthRect, GUIContent.none, width.intValue, 1, 4);

                            propertyRect.y += propSpace;

                            // HEIGHT
                            Rect heightRect = propertyRect;
                            EditorGUI.LabelField(heightRect, "Height");
                            heightRect.xMin += kFieldLabelWidth;
                            var height = properties["Height"];
                            height.intValue = (ushort)EditorGUI.Slider(heightRect, GUIContent.none, height.intValue, 1, 4);

                            propertyRect.y += propSpace;

                            // PADDING
                            Rect paddingRect = propertyRect;
                            EditorGUI.LabelField(paddingRect, "Padding");
                            paddingRect.xMin += kFieldLabelWidth;
                            var padding = properties["Padding"];
                            padding.floatValue = EditorGUI.FloatField(paddingRect, GUIContent.none, padding.floatValue);

                            propertyRect.y += propSpace;

                            // ORIENTATION
                            Rect orientationRect = propertyRect;
                            EditorGUI.LabelField(orientationRect, "Orientation");
                            orientationRect.xMin += kFieldLabelWidth;
                            orientationRect.xMax -= 100f;
                            var orientation = properties["Orientation"];
                            orientation.enumValueIndex = (int)(Orientation)EditorGUI.EnumPopup(orientationRect, (Orientation)orientation.enumValueIndex);

                            // FLIP DIRECTION
                            using (new EditorGUI.DisabledGroupScope(itemCache.orientation.enumValueIndex == 0))
                            {
                                Rect flipDirectionRect = propertyRect;
                    ...
                    if (isUsable && EditorDrawing.BeginFoldoutBorderLayout(itemCache.usableSettings, new GUIContent("Usable Settings")))
                    {
                        ...
                        else if (usableTypeEnum == UsableType.Custom)
                        {
                            SerializedProperty removeOnUse = itemCache.usableSettings.FindPropertyRelative("removeOnUse");
                            EditorGUILayout.PropertyField(removeOnUse);
                        }
        ...
        private void DrawCombination(BuilderItem item, int index)
        {
            ...
            if (EditorDrawing.BeginFoldoutBorderLayout(element, headerGUI, out Rect headerRect, 18f, false))
            {
                ...
                bool isCrafting = itemType.enumValueIndex == 1;
                ...
                using (new EditorDrawing.BorderBoxScope())
                {
                    ...
                    using (new EditorGUI.DisabledGroupScope(!isCrafting))
                    {
            ...
            if (GUI.Button(minusRect, EditorUtils.Styles.TrashIcon, EditorStyles.iconButton))
            {
                ...
                popup.AddItem(new GUIContent("Delete"), false, () =>
                {
                    ...
                    Debug.Log($"[Inventory Builder] Removed combination {index} from {item.Title}");
                });
                ...
                if (partnerItem != null)
                {
                    popup.AddItem(new GUIContent("Delete With Mirrored"), false, () =>
                    {
                        ...
                        Debug.Log($"[Inventory Builder] Removed combination {index} from {item.Title} and its mirrored from {partnerItem.Title}");
                    });
                }
                else
                {
