...
{
    public class OptionsTreeView : TreeView<int>
    {
        ...
        internal class OptionTreeViewItem : TreeViewItem<int>
        {
        ...
        internal class SectionTreeViewItem : TreeViewItem<int>
        {
        ...
        public OptionsTreeView(TreeViewState<int> viewState, BuilderData builderData) : base(viewState)
        {
        ...
        {
            var root = new TreeViewItem<int> { id = 0, depth = -1, displayName = "Options" };
            int id = 1;
        ...
        public override void OnGUI(Rect rect)
        {
            ...
            headerRect.xMin = headerRect.xMax - EditorGUIUtility.singleLineHeight -
        ...
        protected override void RowGUI(RowGUIArgs args)
        {
            ...
            Rect labelRect = new(rect.x + GetContentIndent(item), rect.y, rect.width - GetContentIndent(item),
                rect.height);
                            ...
                            if (draggedItem != null && draggedItem is OptionTreeViewItem item)
                            {
                                if (args.performDrop)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        OnMoveItemToSectionAt(parentSection.Data.GUID, item.Data.GUID,
                                            section1.Data.GUID, args.insertAtIndex);
                                    }
                                }
        ...
        private void OnMoveItem(string sectionGuid, string itemGuid, int newIndex)
        {
            ...
            // compute target index while item is still in the list
            int insertTo = newIndex > itemIndex ? newIndex - 1 : newIndex;
            insertTo = Mathf.Clamp(insertTo, 0, section.Items.Count - 1);

            if (itemIndex == insertTo)
                return;
        ...
        private void OnMoveItemToSection(string fromSectionGuid, string itemGuid, string toSectionGuid)
        {
            ...
            if (fromSectionGuid == toSectionGuid)
            {
                var sectionIndex = builderData.Target.Sections.FindIndex(s => s.Section.GUID == toSectionGuid);
                if (sectionIndex < 0) return;

                var section = builderData.Target.Sections[sectionIndex];
                OnMoveItem(toSectionGuid, itemGuid, section.Items.Count);
                return;
            }
        ...
        {
            // same section -> use in-section reorder logic
            if (fromSectionGuid == toSectionGuid)
            {
                OnMoveItem(toSectionGuid, itemGuid, newIndex);
                return;
            }
            ...
            // add item to destination section
            var toSection = builderData.Target.Sections[toSectionIndex];
            int insertTo = Mathf.Clamp(newIndex, 0, toSection.Items.Count);
            toSection.Items.Insert(insertTo, item);
        }
        ...
        private void OnMoveSection(string sectionGuid, int newIndex)
        {
            ...
            // compute target index using section count, not item count
            int insertTo = newIndex > sectionIndex ? newIndex - 1 : newIndex;
            insertTo = Mathf.Clamp(insertTo, 0, builderData.Target.Sections.Count - 1);

            if (sectionIndex == insertTo)
                return;
