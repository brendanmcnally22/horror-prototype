...
namespace ThunderWire.Editors
{
    public class PropertyCollection : Dictionary<string, SerializedProperty>
    {
        ...
        public void GetEnum<T>(string propertyName, out T enumValue) where T : Enum
        {
            enumValue = default;
            if (TryGetValue(propertyName, out SerializedProperty property))
            {
                enumValue = (T)Enum.ToObject(typeof(T), property.enumValueIndex);
            }
        }
        ...
        public void DrawBacking(string propertyName, GUIContent label)
        {
            propertyName = $"<{propertyName}>k__BackingField";
            if (TryGetValue(propertyName, out SerializedProperty property))
