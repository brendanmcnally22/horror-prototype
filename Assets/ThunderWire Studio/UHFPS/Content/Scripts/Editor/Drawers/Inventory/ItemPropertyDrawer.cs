            ...
            EditorGUI.BeginProperty(position, label, property);
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        itemTitleGUI.tooltip = "Item data is outdated! Click the refresh button to update.";
                    }
                ...
                Rect iconPreviewSlot = position;
                float iconPreviewSize = 3f + EditorGUIUtility.singleLineHeight * 2f;

                iconPreviewSlot.width = iconPreviewSize;
                iconPreviewSlot.height = iconPreviewSize;
                iconPreviewSlot.y += 2f;
                iconPreviewSlot.x += 2f;

                GUI.Box(iconPreviewSlot, GUIContent.none, EditorStyles.helpBox);

                if (itemIcon != null)
                {
                    const float kSlotMargin = 6f;

                    Vector2 slotSize = iconPreviewSlot.size;
                    slotSize -= new Vector2(kSlotMargin, kSlotMargin);

                    slotSize.x = Mathf.Max(0.001f, slotSize.x);
                    slotSize.y = Mathf.Max(0.001f, slotSize.y);

                    Vector2 iconSize = new Vector2(itemIcon.width, itemIcon.height);

                    Vector2 scaleRatio = slotSize / iconSize;
                    float scaleFactor = Mathf.Min(scaleRatio.x, scaleRatio.y);

                    Vector2 drawSize = iconSize * scaleFactor;

                    // Center inside the slot rect
                    Rect iconPreviewRect = new Rect(
                        iconPreviewSlot.center.x - drawSize.x * 0.5f,
                        iconPreviewSlot.center.y - drawSize.y * 0.5f,
                        drawSize.x,
                        drawSize.y
                    );

                    EditorDrawing.DrawTransparentTexture(iconPreviewRect, itemIcon);
                }
                else
                {
                    Rect iconPreviewRect = iconPreviewSlot;
                    iconPreviewRect.width -= 18f;
                    iconPreviewRect.height -= 18f;
                    iconPreviewRect.y += 9f;
                    iconPreviewRect.x += 9f;

                    Texture missingImage = Resources.Load<Texture2D>("EditorIcons/no_icon");
                    EditorDrawing.DrawTransparentTexture(iconPreviewRect, missingImage);
                }

                Rect itemTitleRect = iconPreviewSlot;
