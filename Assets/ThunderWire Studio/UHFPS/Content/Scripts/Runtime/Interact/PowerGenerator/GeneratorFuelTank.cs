...
namespace UHFPS.Runtime
{
    ...
    public class GeneratorFuelTank : MonoBehaviour, IInteractStart, IInteractStop, IInteractTimed
    {
        ...
        public string FuelProperty = "fuel";
        ...
        public void InteractStart()
        {
            ...
            if (Inventory.Instance.ContainsItemMany(FuelItem, out var items))
            {
                ...
                {
                    var customData = item.inventoryItem.CustomData;
                    float fuel = customData.GetValue<float>(FuelProperty);
                    itemWithLiters.Add(item.inventoryItem, fuel);
                }
        ...
        public void InteractTimed()
        {
            ...
            foreach (var item in requiredCanisters)
            {
                if (item.Value > 0)
                {
                    var customData = item.Key.CustomData;
                    customData.SetValue(FuelProperty, item.Value);
                }
                else
                {
