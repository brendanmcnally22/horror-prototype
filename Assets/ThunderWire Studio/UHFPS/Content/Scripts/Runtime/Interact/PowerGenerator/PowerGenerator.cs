...
namespace UHFPS.Runtime
{
    public class PowerGenerator : MonoBehaviour, ISaveable
    {
        ...
        [Range(0.01f, 1f)]
        [Tooltip("How efficiently the generator converts fuel energy into electrical power. Higher values reduce fuel consumption under load.")]
        public float GeneratorEfficiency = 0.4f;
        [Tooltip("Energy content of the fuel per liter. Higher values make the same amount of fuel last longer.")]
        public float FuelCalorificValue = 35f;
        [Tooltip("Baseline fuel consumed per hour while the generator is running, even with no electrical load.")]
