...
namespace UHFPS.Runtime
{
    ...
    public static class SaveableExtensions
    {
        ...
        /// Add basic transform properties to a storable collection. (position, rotation, scale)
        /// </summary>
        public static void StoreTransform(this StorableCollection storableCollection, Transform transform, bool includeScale = false)
        {
            storableCollection["position"] = transform.position.ToSaveable();
            storableCollection["rotation"] = transform.eulerAngles.ToSaveable();
            if (includeScale) storableCollection["scale"] = transform.localScale.ToSaveable();
        }
        ...
        public static void StoreRigidbody(this StorableCollection storableCollection, Rigidbody rigidbody)
        {
            storableCollection["rb_kinematic"] = rigidbody.isKinematic;
            storableCollection["rb_gravity"] = rigidbody.useGravity;
            storableCollection["rb_freeze"] = rigidbody.freezeRotation;
        }

        public static void LoadRigidbody(this JToken token, Rigidbody rigidbody)
        {
            if (token["rb_kinematic"] != null)
            {
                rigidbody.isKinematic = token["rb_kinematic"].ToObject<bool>();
            }
            if (token["rb_gravity"] != null)
            {
                rigidbody.useGravity = token["rb_gravity"].ToObject<bool>();
            }
            if (token["rb_freeze"] != null)
            {
                rigidbody.freezeRotation = token["rb_freeze"].ToObject<bool>();
            }
        }
    }
}