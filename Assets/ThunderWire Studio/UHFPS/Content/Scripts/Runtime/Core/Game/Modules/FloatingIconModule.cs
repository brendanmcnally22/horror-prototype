...
namespace UHFPS.Runtime
{
    ...
    public class FloatingIconModule : ManagerModule
    {
        ...
        public void DestroyFloatingIcon(GameObject gameObject)
        {
            for (int i = uiFloatingIcons.Count - 1; i >= 0; i--)
            {
                FloatingIconData data = uiFloatingIcons[i];
                if (data == null)
                    continue;

                if (data.targetObject == gameObject)
                {
                    if (data.floatingIcon != null)
                        data.floatingIcon.FadeOut(FadeOutTime);
        ...
        private bool VisibleByCamera(GameObject obj, LayerMask cullLayers)
        {
            if (obj != null)
            {
                bool linecastResult = Physics.Linecast(PlayerPresence.PlayerCamera.transform.position, obj.transform.position, out RaycastHit hit, cullLayers, QueryTriggerInteraction.Ignore);
