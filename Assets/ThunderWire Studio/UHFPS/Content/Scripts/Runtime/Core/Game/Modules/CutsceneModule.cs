...
namespace UHFPS.Runtime
{
    ...
    public class CutsceneModule : ManagerModule
    {
        ...
        public float GroundTestHeight = 1f;
        ...
        private void ApplyPlayerTransform(Transform endTransform)
        {
            ...
            else if (currentTrigger.ShiftPlayerWhenCutsceneEnds)
            {
                newPosition += lookForward.normalized * 0.5f;
            }

            if (currentTrigger.GetPlayerFeetPositionFromEndPosition)
                newPosition = GetRealPlayerPosition(newPosition);
            
            Vector2 newLook = new(currentLook.y, 0f);
            PlayerPresence.SetPlayerPositionAndLook(newPosition, newLook);
        }

        private Vector3 GetRealPlayerPosition(Vector3 position)
        {
            PlayerStateMachine player = PlayerPresence.PlayerManager.PlayerStateMachine;
            if (player == null) return position;

            Vector3 origin = position + Vector3.up * GroundTestHeight;
            if (Physics.Raycast(origin, Vector3.down, out RaycastHit hit, GroundTestHeight * 2f, GroundTestMask))
            {
                // convert hit point to feet position
                return player.ToFeetPosition(hit.point);
            }
