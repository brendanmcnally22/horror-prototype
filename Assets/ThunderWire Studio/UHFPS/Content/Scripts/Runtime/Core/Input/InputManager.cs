...
namespace UHFPS.Input
{
    ...
    public class InputManager : Singleton<InputManager>
    {
        ...
        private readonly HashSet<string> pressedActions = new();
        ...
        /// <summary>
        /// Check whether the input is pressed once for the specified owner.
        /// </summary>
        public static bool ReadInputOnce<T>(UnityEngine.Object owner, string actionName, out T value) where T : struct
        {
            string inputKey = GetInputKey(owner, actionName);
            InputAction inputAction = Action(actionName);

            if (inputAction.IsPressed())
            {
                if (Instance.pressedActions.Add(inputKey))
                {
                    value = inputAction.ReadValue<T>();
                    return true;
                }
        ...
        /// <summary>
        /// Read the input as a button once for the specified Unity object.
        /// </summary>
        public static bool ReadButtonOnce(UnityEngine.Object owner, string actionName)
        {
            ...
            if (ReadButton(actionName))
                return Instance.pressedActions.Add(inputKey);
        ...
        /// <summary>
        /// Read the input as a button once using a custom owner key.
        /// </summary>
        public static bool ReadButtonOnce(string ownerKey, string actionName)
        {
            string inputKey = GetInputKey(ownerKey, actionName);
        ...
        /// <summary>
        /// Read the input button as a toggle for the specified Unity object.
        /// </summary>
        public static bool ReadButtonToggle(UnityEngine.Object owner, string actionName)
        {
            string inputKey = GetInputKey(owner, actionName);

            if (ReadButtonOnce(owner, actionName))
            {
        ...
        /// <summary>
        /// Read the input button as a toggle using a custom owner key.
        /// </summary>
        public static bool ReadButtonToggle(string ownerKey, string actionName)
        {
            string inputKey = GetInputKey(ownerKey, actionName);

            if (ReadButtonOnce(ownerKey, actionName))
            {
                if (Instance.toggledActions.ContainsKey(inputKey))
                    Instance.toggledActions.Remove(inputKey);
                else
        ...
        /// <summary>
        /// Set the button toggle state for the specified Unity object.
        /// </summary>
        public static void SetButtonToggle(UnityEngine.Object owner, string actionName, bool state)
        {
            string inputKey = GetInputKey(owner, actionName);

            if (state) Instance.toggledActions[inputKey] = true;
            else Instance.toggledActions.Remove(inputKey);
        }

        /// <summary>
        /// Set the button toggle state using a custom owner key.
        /// </summary>
        public static void SetButtonToggle(string ownerKey, string actionName, bool state)
        {
            string inputKey = GetInputKey(ownerKey, actionName);
        ...
        /// <summary>
        /// Reset the toggled button for the specified Unity object.
        /// </summary>
        public static void ResetToggledButton(UnityEngine.Object owner, string actionName)
        {
            string inputKey = GetInputKey(owner, actionName);
            Instance.toggledActions.Remove(inputKey);
        }
        ...
        /// <summary>
        /// Reset the toggled button using a custom owner key.
        /// </summary>
        public static void ResetToggledButton(string ownerKey, string actionName)
        {
            string inputKey = GetInputKey(ownerKey, actionName);
            Instance.toggledActions.Remove(inputKey);
        }
        ...
        private static string GetOwnerKey(UnityEngine.Object owner)
        {
            ...
            int referenceHash = System.Runtime.CompilerServices
                .RuntimeHelpers
                .GetHashCode(owner);

            return owner.GetType().FullName + "." + referenceHash;
        }

        private static string GetInputKey(UnityEngine.Object owner, string actionName)
        {
            return GetOwnerKey(owner) + "." + actionName;
        }

        private static string GetInputKey(string ownerKey, string actionName)
        {
            if (string.IsNullOrEmpty(ownerKey))
                throw new ArgumentException("Owner key cannot be null or empty.", nameof(ownerKey));

            return ownerKey + "." + actionName;
        }
