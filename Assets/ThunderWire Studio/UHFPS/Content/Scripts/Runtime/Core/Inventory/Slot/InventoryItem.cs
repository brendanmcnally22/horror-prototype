    ...
    public class InventoryItem : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IPointerClickHandler
    {
        ...
        private void GetInput()
        {
            if (isOver || isMoving)
            {
                ...
                // item movement input
                if (InputManager.ReadButtonOnce(this, Controls.INVENTORY_ITEM_MOVE))
                {
                    if (!isMoving)
                    {
                        if(Inventory.ContainerOpened)
            ...
            // item rotation input
            if (InputManager.ReadButtonOnce(this, Controls.INVENTORY_ITEM_ROTATE) && isMoving && !isRotating && Item.Width != Item.Height)
            {
                Inventory.PlayInventorySound(InventorySound.ItemMove);
