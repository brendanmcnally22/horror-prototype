...
namespace UHFPS.Runtime
{
    ...
    public class PlayerStateMachine : PlayerComponent
    {
        ...
        [Serializable]
        public sealed class DynamicHeightSettings
        {
            public bool EnableDynamicHeight = false;
            public bool EnableDebugMode = false;
            
            [Tooltip("When enabled, dynamic height will work only if the player is inside a specific trigger.")]
            public bool TriggerBased = false;

            [Tooltip("Raycasting mode that will be used to detect height changes.")]
            public DynamicHeightMode HeightRayMode = 0;
            [Tooltip("Layer mask for the height raycasts.")]
            public LayerMask HeightRayMask;

            // Ray settings
            [Tooltip("Radius grow for the sphere cast when using SphereCast mode.")]
            public float SphereCastRadiusOffset = 0.2f;
            [Tooltip("Radius of the sphere cast rays used in raycasting.")]
            public float RaySphereRadius = 0.05f;
            [Tooltip("Length of the height raycasts.")]
            public float HeightRayLength = 2f;
            [Tooltip("Vertical offset of the height ray origin from the controller feet.")]
            public float HeightRayOffset = 0.5f;
            [Tooltip("Forward offset of the height ray origin from the controller feet.")]
            public float HeightRayFront = 0.5f;

            // Height settings
            [Tooltip("Camera offset applied when changing the controller height.")]
            public Vector3 CameraOffset = new (0f, -0.1f, 0f);
            [Tooltip("Minimum and maximum height of the character controller that can be set dynamically.")]
            public MinMax ControllerHeight = new(1f, 1.35f);
            [Tooltip("Height offset from top position of the detected surface to apply to the character controller.")]
            public float HeightTargetOffset = 0.1f;
        }
        ...
        public DynamicHeightSettings DynamicHeight;
        ...
        public bool EnableDynamicHeight { get; set; }
        ...
        /// <summary>
        /// Check if the player is in crouching state.
        /// </summary>
        public bool StateCrouched
        {
            get => IsCurrent(CROUCH_STATE);
        }
        ...
        /// <summary>
        /// Increase/Descrease player stamina by amount.
        /// </summary>
        public void AddStamina(float amount)
        {
            if (!PlayerFeatures.EnableStamina || PlayerManager.PlayerHealth.IsDead) 
                return;

            // reset stamina regen time
            staminaRegenTime = PlayerStamina.RegenerateAfter;
        ...
        /// <summary>
        /// Calculate the player pivot position based on the given ground position and the current controller state.
        /// </summary>
        /// <returns>Returns the CharacterController pivot (transform.position).</returns>
        public Vector3 ToFeetPosition(Vector3 groundPosition)
        {
            float skinWidth = Controller.skinWidth;
            Vector3 feetOffset = FeetOffset;
            return groundPosition - feetOffset + Vector3.up * skinWidth;
        }
        ...
        /// </summary>
        public void ChangeState<TState>(bool force = false) where TState : PlayerStateAsset
        {
            if (playerStates.TryGetValue(typeof(TState), out State state))
            {
                if (!force && !isEnabled && !state.fsmState.CanTransitionWhenDisabled)
                    return;
        ...
        /// Change player FSM state.
        /// </summary>
        public void ChangeState(Type nextState, bool force = false)
        {
        ...
        /// Change player FSM state.
        /// </summary>
        public void ChangeState(string nextState, bool force = false)
        {
            if (playerStates.TryGetValue(nextState, out State state))
            {
                if (!force && !isEnabled && !state.fsmState.CanTransitionWhenDisabled)
        ...
        private void OnDrawGizmosSelected()
        {
            ...
            if (DynamicHeight.EnableDynamicHeight && DynamicHeight.EnableDebugMode)
            {
                Vector3 rayOrigin = ControllerFeet + Vector3.up * DynamicHeight.HeightRayOffset;
                Vector3 forward = !Application.isPlaying ? transform.forward : LookController.LookForward;
                Vector3 sphereOrigin = rayOrigin + forward.normalized * DynamicHeight.HeightRayFront;

                if (DynamicHeight.HeightRayMode == DynamicHeightMode.Frontal1Ray)
                {
                    DrawDynamicHeightSphere(sphereOrigin, DynamicHeight.RaySphereRadius);
                    DrawDynamicHeightRay(rayOrigin, forward);
                }
                else if (DynamicHeight.HeightRayMode == DynamicHeightMode.Frontal3Rays)
                {
                    DrawDynamicHeightSphere(sphereOrigin, DynamicHeight.RaySphereRadius);
                    DrawDynamicHeightRay(rayOrigin, forward);
                    DrawDynamicHeightRay(rayOrigin, Quaternion.Euler(0, 60, 0) * forward);
                    DrawDynamicHeightRay(rayOrigin, Quaternion.Euler(0, -60, 0) * forward);
                }
                else if (DynamicHeight.HeightRayMode == DynamicHeightMode.AllAround6Rays)
                {
                    DrawDynamicHeightSphere(sphereOrigin, DynamicHeight.RaySphereRadius);
                    DrawDynamicHeightRay(rayOrigin, forward);
                    DrawDynamicHeightRay(rayOrigin, Quaternion.Euler(0, 60, 0) * forward);
                    DrawDynamicHeightRay(rayOrigin, Quaternion.Euler(0, 120, 0) * forward);
                    DrawDynamicHeightRay(rayOrigin, Quaternion.Euler(0, 180, 0) * forward);
                    DrawDynamicHeightRay(rayOrigin, Quaternion.Euler(0, -60, 0) * forward);
                    DrawDynamicHeightRay(rayOrigin, Quaternion.Euler(0, -120, 0) * forward);
                }
                else if (DynamicHeight.HeightRayMode == DynamicHeightMode.SphereCast)
                {
                    float radius = Controller.radius + DynamicHeight.SphereCastRadiusOffset;
                    sphereOrigin = ControllerFeet + Vector3.up * radius;
                    DrawDynamicHeightSphere(sphereOrigin, radius);
                }
            }
        }

        private void DrawDynamicHeightSphere(Vector3 origin, float radius)
        {
            Gizmos.color = Color.cyan;
            Gizmos.DrawWireSphere(origin, radius);
        }

        private void DrawDynamicHeightRay(Vector3 origin, Vector3 dir)
        {
            Vector3 rayStart = origin + dir.normalized * DynamicHeight.HeightRayFront;

            Gizmos.color = Color.yellow;
            Gizmos.DrawLine(origin, rayStart);
            Gizmos.DrawSphere(rayStart, 0.02f);

            Gizmos.color = Color.green;
            Gizmos.DrawLine(rayStart, rayStart + Vector3.up * DynamicHeight.HeightRayLength);
        }
