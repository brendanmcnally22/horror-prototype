...
namespace UHFPS.Runtime
{
    ...
    public class MotionController : PlayerComponent
    {
        ...
        /// <summary>
        /// Set head bobbing motion active state for all motion states.
        /// </summary>
        public void SetHeadBobbing(bool value)
        {
            if (MotionBlender.IsInitialized)
            {
                foreach (var state in MotionBlender.Instance.StateMotions)
                {
                    foreach (var motion in state.Motions)
                    {
                        if (motion is BobMotion bobMotion)
                            bobMotion.Weight = value ? 1f : 0f;
                    }
                }
            }
        }
